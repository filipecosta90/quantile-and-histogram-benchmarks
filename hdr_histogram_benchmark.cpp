...
static void BM_hdr_add_lognormal_dist(benchmark::State &state)
{
    const int64_t precision = state.range(0);
  const int64_t max_value = state.range(1);
  struct hdr_histogram *histogram1, *histogram2;
  hdr_init(min_value, max_value, precision, &histogram1);
  hdr_init(min_value, max_value, precision, &histogram2);
    ...
    std::lognormal_distribution<double> distSamples(max_value,max_value/2);
  
    for (double &i : input)
    {
        i = dist(rng);
        int64_t number = int64_t(distSamples(rng)) + 1;
        number = number > max_value ? max_value : number;
        hdr_record_value(histogram1, number);
    }
    for (double &i : input)
    {
        i = dist(rng);
        int64_t number = int64_t(distSamples(rng)) + 1;
        number = number > max_value ? max_value : number;
        hdr_record_value(histogram2, number);
    }
